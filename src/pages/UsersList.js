    ...
    if (activemodalUserid) {
      ...
      axios
        ...
        .catch((error) => {
          ...
          setShowActiveModal(false); // Close the modal on error as well
        });
    }
  };

  function fetchUser() {
    axios.get(API_URL + "/get_all_student_data").then((response) => {
      if (response.data.success) {
        setTotalUser(response.data.user_arr);
      } else {
        setTotalUser([])
      }
    }).catch((error) => {
    ...
    <><div className="container mt-5">
          ...
          <table className="table">
            <thead className='table-head'>
              <tr>
                ...
                <th>Status</th>
                <th>Referral Code</th>
                <th>Join Referral Code Count</th>
              ...
              {currentUsers.length > 0 ? (
                currentUsers.map((user, index) => (
                  <tr key={user.sn}>
                    ...
                    <td><img alt="user image" src={user.image
                    ...
                    <td>{user.referral_code || "NA"}</td>
                    <td>{user.join_referral_code || 0}</td>
      ...
      >
         <Modal.Header closeButton style={{ width: '100%' }}>
          <Modal.Title style={{ fontSize: '20px' }}>Activate/Deactivate User</Modal.Title>
        </Modal.Header>
